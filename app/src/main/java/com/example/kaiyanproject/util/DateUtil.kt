...
object DateUtil {
        ...
        if (timePast > -MINUTE) { // 采用误差一分钟以内的算法，防止客户端和服务器时间不同步导致的显示问题
            when {
                ...
                timePast < WEEK -> {
                    ...
                    return "$pastDays ${R.string.days_ago.resString}"
                }
                ...
                timePast < MONTH -> {
                    ...
                    return "$pastDays ${R.string.weeks_ago.resString}"
                }
    ...
    fun getDate(dateMillis: Long, pattern: String = "yyyy-MM-dd") =
        SimpleDateFormat(pattern, Locale.getDefault()).format(Date(dateMillis))

    private fun getDateAndTime(dateMillis: Long) =
        SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault()).format(Date(dateMillis))

    private fun getDateAndHourMinuteTime(dateMillis: Long) =
